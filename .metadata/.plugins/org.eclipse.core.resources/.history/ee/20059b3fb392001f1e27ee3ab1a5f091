...
public class Map {
    ...
    public boolean isCellEmpty(int x, int y) {
        return board[y][x] != '#' && !(x == pacmanX && y == pacmanY) &&
               !isOccupiedByGhost(x, y); // 유령 위치 확인 추가
    }

    private boolean isOccupiedByGhost(int x, int y) {
        for (int i = 0; i < 3; i++) {
    ...
    public Map() {
        ...
        itemX = 5; // 동전 위치
        itemY = 5; // 동전 위치
        star = new Star(this); // Star 객체 초기화

        // 유령의 시작 위치를 중앙에 가로로 배치
        ghostX[0] = 6; ghostY[0] = 5; // 유령 1
        ghostX[1] = 7; ghostY[1] = 5; // 유령 2
        ghostX[2] = 8; ghostY[2] = 5; // 유령 3
    }
    ...
    public int[][] getStarPositions() {
        return star.getStarPositions(); // 별의 위치를 반환
    }
}
